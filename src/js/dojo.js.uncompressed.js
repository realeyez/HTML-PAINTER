    ...
    if(1){
        ...
        computeMapProg = function(map, dest, packName){
            ...
            for(i = 0; i < dest.length;){
                item = dest[i++];
                item[2] = new RegExp("^" + item[0].replace(/([\.$?*|{}\(\)\[\]\\\/\+^])/g, function(c){
                    return "\\" + c;
                }) + "(\/|$)");
        ...
        config = function(config, booting){
            ...
            forEach(config.aliases, function(pair){
                if(isString(pair[0])){
                    pair[0] = new RegExp("^" + pair[0].replace(/([\.$?*|{}\(\)\[\]\\\/\+^])/g, function(c){
                        return "\\" + c;
                    }) + "$");
                }
                aliases.push(pair);
            ...
            function(dojo, require, has, array, config, lang, xhr) {
                ...
                if(1){
                    ...
                    fixup= function(url, preAmdResult, amdResult){
                        ...
                        return preAmdResult ? (/nls\/[^\/]+\/[^\/]+$/.test(url) ? preAmdResult : {
                            root:preAmdResult, 
                            _v1x:1
                        }) : amdResult;
                    },
                    ...
                    thisModule.getLocalization= function(moduleName, bundleName, locale){
                        ...
                        load(l10nName, (1 && !require.isXdUrl(require.toUrl(l10nName + ".js")) ? syncRequire : require), function(result_){
                            result= result_;
                        });
                        return result;
                    };
